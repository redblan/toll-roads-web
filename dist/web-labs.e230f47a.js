class u{constructor(u){this.parent=u}getHTML(u){return`
        ...
        `}render(u,t,e,d){this.parent.insertAdjacentHTML("beforeend",this.getHTML(u));let a=this.parent.querySelectorAll(".route-card"),i=a[a.length-1];i.querySelector(".btn-details").addEventListener("click",u=>{u.stopPropagation(),t()}),i.addEventListener("click",t),i.querySelector(".btn-edit").addEventListener("click",u=>{u.stopPropagation(),d()}),i.querySelector(".btn-delete").addEventListener("click",u=>{u.stopPropagation(),e()})}}class t{constructor(u){this.parent=u}getHTML(u){let t=u.sections||[],e=t.map((t,e)=>`
                ...
                <div class="container">
                    <div class="d-flex gap-2 mb-4 flex-wrap align-items-center">
                        <input
                            ...
                            placeholder="\u{41F}\u{43E}\u{438}\u{441}\u{43A} \u{43F}\u{43E} \u{43D}\u{430}\u{437}\u{432}\u{430}\u{43D}\u{438}\u{44E} \u{442}\u{440}\u{430}\u{441}\u{441}\u{44B}..."
                        >
                        <button class="btn btn-search" id="btn-search">\u{1F50D} \u{41D}\u{430}\u{439}\u{442}\u{438}</button>
        ...
        `}async loadTollRoads(t=""){let e=document.getElementById("routes-grid"),d=document.getElementById("toll-road-loading"),i=document.getElementById("toll-road-error");e.innerHTML="",d.style.display="block",i.style.display="none";try{let i=await a(t);d.style.display="none",i.forEach(t=>{new u(e).render(t,()=>this.clickCard(t.id),()=>this.deleteRoute(t.id),()=>this.editRoute(t.id))})}catch(u){d.style.display="none",i.style.display="block",console.error("fetch error:",u)}}clickCard(u){new o(this.parent,u).render()}editRoute(u){new r(this.parent,u).render()}async deleteRoute(u){try{await s(u),this.loadTollRoads(document.getElementById("toll_road_filter")?.value||"")}catch(u){console.error("Ошибка удаления:",u.message)}}render(){this.parent.innerHTML="",this.parent.insertAdjacentHTML("beforeend",this.getHTML()),document.getElementById("btn-home").addEventListener("click",()=>{new c(this.parent).render()}),document.getElementById("btn-add-route").addEventListener("click",()=>{new r(this.parent,null).render()}),document.getElementById("btn-search").addEventListener("click",()=>{let u=document.getElementById("toll_road_filter").value;this.loadTollRoads(u)}),document.getElementById("toll_road_filter").addEventListener("keydown",u=>{"Enter"===u.key&&this.loadTollRoads(u.target.value)}),this.loadTollRoads()}}new c(document.getElementById("root")).render();
//# sourceMappingURL=web-labs.e230f47a.js.map
